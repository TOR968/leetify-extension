...
const CONTAINER = '.leetify-extension-container';
...
async function detectActiveInjector(): Promise<Injector> {
	...
	try {
		...
		console.error('[Leetify] detect injector failed:', e);
	}
	return 'none';
}
...
async function setupCommunityInjection() {
	const CDP = getCDP();
	if (!CDP) { console.error('[Leetify] No CDP available'); return; }
	...
	reloadOpenProfiles = async () => {
		for (const t of await getProfileTargets(CDP)) {
			...
			catch (e) { console.error('[Leetify] reload error:', e); }
		}
	};
	...
	const processTarget = (targetInfo: any) => {
		...
		pending.set(targetId, setTimeout(() => {
			...
			injectIntoTarget(targetId).catch((e: any) => console.error('[Leetify] injection error:', e));
		}, 200));
	};
	...
	return (
		<div style={{ padding: '16px' }}>
			...
			<div style={{ fontSize: '12px', lineHeight: '1.4', color: '#969696' }}>
				Open Leetify in your system browser instead of Steam's built-in browser. Open profile pages reload automatically when you change this.
			</div>
		</div>
	);
};
...
export default definePlugin(() => {
	setupCommunityInjection().catch(e => console.error('[Leetify] setup error:', e));
	return { name: 'leetify-extension', title: 'Leetify Extension', icon: <LeetifyIcon />, content: <Settings /> };
});
